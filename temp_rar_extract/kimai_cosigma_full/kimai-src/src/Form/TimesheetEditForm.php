...
class TimesheetEditForm extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        if (isset($options['data']) && $options['data'] instanceof Timesheet) {
            ...
            // Prefill "On-site" based on kimai2_working_times.onesite for the begin date of this timesheet.
            // This makes the UX clearer when the user re-opens a timesheet form for the same day.
            if (null !== ($begin = $entry->getBegin()) && null !== ($user = $entry->getUser())) {
                $day = new \DateTimeImmutable(
                    $begin->format('Y-m-d 00:00:00'),
                    // `User::getTimezone()` returns a string timezone identifier (e.g. "Europe/Hanoi")
                    // so we must not call `->getName()` on it.
                    new \DateTimeZone($user->getTimezone())
                );
                if (null !== ($workingTime = $this->workingTimeRepository->findOneBy(['user' => $user, 'date' => $day]))) {
                    $defaultOnsite = $workingTime->isOnsite();
                }
            }
        }
        ...
        // Incentive tracking (Cosigma): stored in `kimai2_working_times.onesite` by the controller on save.
        $builder->add('onsite', YesNoType::class, [
            'label' => 'On-site',
            'required' => false,
            'mapped' => false,
            'data' => $defaultOnsite,
        ...
        $builder->add('lunch_break', CheckboxType::class, [
            'label' => 'Lunch break (1h)',
            'required' => false,
            'mapped' => false,
            'data' => $isNew,
        ]);
        ...
        $builder->addEventListener(FormEvents::SUBMIT, function (FormEvent $event): void {
            ...
            if (!$form->has('lunch_break')) {
                return;
            }

            $oldBreak = $timesheet->getBreak();
            $lunch = $form->get('lunch_break')->getData() === true;
            $breakSeconds = $lunch ? 3600 : 0;
            $timesheet->setBreak($breakSeconds);

            // If a manual duration is used, keep it stable when toggling lunch on/off.
            // We store duration as "working time" (excluding breaks), so switching lunch adjusts the stored duration.
            $duration = $timesheet->getDuration(false);
            if ($duration !== null && $duration > 0 && $oldBreak !== $breakSeconds) {
                $timesheet->setDuration(max(0, $duration + $oldBreak - $breakSeconds));
            }
        });
