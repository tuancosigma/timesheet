...
abstract class TimesheetAbstractController extends AbstractController
{
    ...
    protected function index(TimesheetQuery $query, Request $request, string $route, string $paginationRoute, string $location): Response
    {
        ...
        foreach ($result->getResults() as $entry) {
            if ($entry->getBegin() !== null && $entry->getUser() !== null) {
                $dayId = $entry->getBegin()->format('Y-m-d');
                $user = $entry->getUser();
                $key = $user->getId() . '_' . $dayId;

                if (!isset($onsiteMap[$key])) {
                    $day = new \DateTimeImmutable($dayId . ' 00:00:00', new \DateTimeZone($user->getTimezone()));
                    $wt = $this->workingTimeRepository->findOneBy(['user' => $user, 'date' => $day]);
                    $onsiteMap[$key] = ($wt !== null && $wt->isOnsite());
                }
            }
        }
        ...
        $table->addColumn('onsite', ['class' => 'text-center alwaysVisible w-min', 'title' => 'On-site', 'orderBy' => false]);
    ...
    protected function edit(Timesheet $entry, Request $request): Response
    {
        ...
        if ($editForm->isSubmitted() && $editForm->isValid()) {
            try {
                ...
                $this->updateWorkingTimeOnsiteFromTimesheet($entry, $editForm);
    ...
    protected function create(Request $request): Response
    {
        ...
        if ($createForm->isSubmitted() && $createForm->isValid()) {
            try {
                $this->service->saveTimesheet($entry);
                $this->updateWorkingTimeOnsiteFromTimesheet($entry, $createForm);
                $this->flashSuccess('action.update.success');
    ...
    private function updateWorkingTimeOnsiteFromTimesheet(Timesheet $timesheet, FormInterface $form): void
    {
        if (!$form->has('onsite')) {
            return;
        }

        $onsiteValue = $form->get('onsite')->getData();
        $onsite = $onsiteValue === true; // treat null/false as off
        ...
        // Report cosigma only counts onsite on expected work days.
        $expected = $this->isExpectedWorkDay($user, $day);
        $desiredOnsite = $expected ? $onsite : false;

        $workingTime = $this->workingTimeRepository->findOneBy(['user' => $user, 'date' => $day]);
        if ($workingTime === null) {
            if (!$desiredOnsite) {
                return;
            }

            $workingTime = new WorkingTime($user, $day);
        }

        // Avoid unnecessary flushes: only write if the value really changes.
        if ($workingTime->isOnsite() === $desiredOnsite) {
            return;
        }

        $workingTime->setOnsite($desiredOnsite);
        $this->entityManager->persist($workingTime);
        $this->entityManager->flush();
    }

    private function isExpectedWorkDay(User $user, \DateTimeInterface $day): bool
    {
        ...
        return $calculator->isWorkDay($day);
    }
    ...
    protected function duplicate(Timesheet $timesheet, Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                if (method_exists($form, 'has') && $form->has('onsite')) {
                    $this->updateWorkingTimeOnsiteFromTimesheet($copyTimesheet, $form);
                }
                $this->flashSuccess('action.update.success');
